...
from zero_shot_model import ZeroShotBase
import json
import math
import torch
import numpy as np
...
def validate(model):
    with torch.no_grad():
        for video_feats, graphs, labels in tqdm(iterate(val_loader, validation=True), desc='Validation'):
...
if __name__ == '__main__':
    ...
    ckpt_file = 'zero_shot_best_{}.pth'.format(str(args.run_id))
    model_ckpt_path = os.path.join(os.getcwd(), ckpt_file)
    logger_filename = 'zero_shot_log_{}.txt'.format(str(args.run_id))
    ...
    hsize = 150
    model = ZeroShotBase(text_model=text_model)
    # model.load_state_dict(torch.load(model_ckpt_path))
    model.cuda()
    # will have unused params for certain samples (StateQuery / RelationQuery)
    # , find_unused_parameters=True
    model = DDP(model, device_ids=[local_rank], find_unused_parameters=True)
    optimizer = optim.AdamW(list(model.parameters()), lr=args.lr, weight_decay=args.weight_decay)
    bce_loss = nn.BCELoss()
