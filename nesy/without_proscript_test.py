...
from proScript.process_dataset import get_output
...
def process_batch(data_batch, label_batch, frames_per_segment):
    ...
    labels = []
    segment_labs_batch = []
    goals_batch, pickup_batch, receptacle_batch = [], [], []
    for filepath, label in zip(data_batch, label_batch):
        traj = json.load(open(os.path.join(filepath, 'traj_data.json'), 'r'))
        trial = filepath.split('/')[-2]
        pickup, _, receptacle, _ = trial.split('-')
        goals_batch.append(traj['task_type'])
        pickup_batch.append(pickup.lower())
        receptacle_batch.append(receptacle.lower())
        ...
        # generate graphs for neg hypotheses
        # for positive hypotheses, use the ground-truth graph
    graphs_batch = np.array([None] * len(hypotheses))
    neg_indices, pos_indices = [], []
    with torch.no_grad():
        for ind, label in enumerate(labels):
            if label == 0:
                neg_indices.append(ind)
            else:
                pos_indices.append(ind)

        # mapping positive hypotheses to ground-truth graphs
        if len(pos_indices) > 0:
            goals_batch = np.array(goals_batch)[pos_indices]
            pickup_batch = np.array(pickup_batch)[pos_indices]
            receptacle_batch = np.array(receptacle_batch)[pos_indices]
            graphs_batch[pos_indices] = [get_output(goal, pickup, receptacle)[1] for goal, pickup, receptacle in
                                         zip(goals_batch, pickup_batch, receptacle_batch)]

        # processing negative hypotheses using proscript model
        if len(neg_indices) > 0:
            inputs_tokenized = tokenizer(np.array(hypotheses)[neg_indices].tolist(),
                                         return_tensors="pt",
                                         padding="longest",
                                         max_length=max_source_length,
                                         truncation=True)
            neg_processed = t5_model.module.generate(inputs_tokenized["input_ids"].cuda(),
                                                     attention_mask=inputs_tokenized["attention_mask"].cuda(),
                                                     max_length=max_target_length,
                                                     do_sample=False)  # greedy generation
            graphs_batch[neg_indices] = tokenizer.batch_decode(neg_processed, skip_special_tokens=True)

    graphs_batch = [ged.pydot_to_nx(graph_str) for graph_str in graphs_batch]
    graphs_batch = (graphs_batch, hypotheses)
...
if __name__ == '__main__':
    ...
    ckpt_file = 'nesy_without_proscript_best_{}.pth'.format(str(args.run_id))
    model_ckpt_path = os.path.join(os.getcwd(), ckpt_file)
    logger_filename = 'nesy_without_proscript_log_test_{}.txt'.format(str(args.run_id))
