...
class NeSyBase(nn.Module):
    ...
    def query(self, segment_ids, nodes, vid_feature):
        ...
        for node, segment_id in zip(nodes, segment_ids):
            ...
            with torch.no_grad():
                text_feats = torch.stack([self.text_model.get_vecs_by_tokens(x) for x in tokenizer_out])
            if query_type == 'StateQuery':
                log_prob = log_prob + self.state_query(torch.cat((text_feats[:2].contiguous().reshape(-1),
                                                                  vid_feature[segment_id].contiguous())))[
                    0].contiguous()
    ...
    def recurse(self, level_ind, start_ind, levels_start, levels, levels_end, vid_feature):
        ...
        # -sys.maxsize + 1
        max_val_level = torch.tensor(-100.)
        ...
        if max_val_level == torch.tensor(-100.):
            breakpoint()
        return max_val_level

    def recurse2(self, levels, levels_end, vid_feature):
        """
        optimized recurse: stores values of sub-problems in L*S array
        # levels = L
        # segments = S
        """
        path_opt = [[None]]*len(levels)
        arr = torch.full((len(levels) + 1, len(vid_feature) + 1), torch.tensor(-100.))
        arr[:, len(vid_feature)] = torch.zeros(len(levels) + 1)
        arr[len(levels), :] = torch.zeros(len(vid_feature) + 1)
            ...
            for segment_ind in range(len(vid_feature) - 1, -1, -1):
                ...
                for perm in all_perm:
                    ...
                    if level_ind <= len(levels) - 1:
                        ...
                        V_opt = self.query(perm, nodes, vid_feature) + V_opt_next
                        if arr[level_ind][start_ind] < V_opt:
                            path_opt[level_ind] = perm
                            arr[level_ind][start_ind] = V_opt
        return arr[0][0].contiguous(), path_opt
    ...
    def forward(self, vid_feats, graphs):
        ...
        for vid_feature, vid_len, graph in zip(vid_features, vid_lens, graphs):
            ...
            # max_val1 = self.recurse(0, 0, levels_start, levels, levels_end, vid_feature)
