...
class BidafAttn(nn.Module):
    ...
    def similarity(self, s1, l1, s2, l2):
        ...
        s_mask = s.new_full(s.size(), 1).bool()  # [B, T1, T2]
    ...
    @classmethod
    def get_u_tile(cls, s, s_mask, s2):
        """
        ...
        # print('a size', a_weight.size())
        a_weight = a_weight.masked_fill(s_mask, 0)  # remove nan from softmax on -inf
    ...
    def forward(self, s1, l1, s2, l2):
        # return s1, None
        s, s_mask = self.similarity(s1, l1, s2, l2)
        u_tile = self.get_u_tile(s, s_mask, s2)
