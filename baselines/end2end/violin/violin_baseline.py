...
def train_epoch(model, train_loader, val_loader, epoch, previous_best_acc):
    ...
    if is_main_process():
        ...
        if val_acc > torch.tensor(previous_best_acc).cuda():
            ...
            if args.finetune:
                # if args.visual_feature_extractor == 'resnet':
                #     torch.save(video_model.module.state_dict(), visual_model_ckpt_path)
                # elif args.visual_feature_extractor == 'i3d':
                #     torch.save(video_model.module.state_dict(), visual_model_ckpt_path)
...
def process_batch(data_batch, label_batch):
    ...
    for filepath, label in zip(data_batch, label_batch):
        ...
        while success:
            if fno % args.sample_rate == 0:
                ...
                if args.howto:
                    video_frames.append(transform_image(img,'video'))
                else:
        ...
        if args.howto:
            ...
            remain_frames = video_frames.shape[0] % 16 
            #print('remain_frames',remain_frames)
            if remain_frames!=0:
                zeros = torch.zeros(( remain_frames, 3, 224, 224,), dtype=torch.uint8, device=video_frames.get_device())
                video_frames = torch.cat((video_frames, zeros), axis=0)
        ...
        if args.visual_feature_extractor == 'resnet':
            ...
            if not args.finetune:
                ...
                video_features_batch.append(video_model(video_frames).view(-1, vid_feat_size))
        elif args.visual_feature_extractor == 'i3d':
    ...
    #print('hypotheses',hypotheses[0].shape,hypotheses)
...
if __name__ == '__main__':
    ...
    if args.finetune:
        ...
        #     all_params += list(video_model.parameters())
        #     # if args.i3d_aggregate == 'rnn':
    ...
    checkpoint_dir = os.path.join(os.path.dirname(__file__), 'checkpoint', args.checkpoint_dir + '_' +str(args.run_id))
    if args.checkpoint_dir != '' and not(os.path.isdir(checkpoint_dir)):
    ...
    if args.resume:
        ...
        if checkpoint_path:
            ...
            if args.finetune:
                video_model.load_state_dict(checkpoint["video_state_dict"])
                ...
                save_checkpoint(
                    {
                        ...
                        "video_state_dict": video_model.state_dict(),
