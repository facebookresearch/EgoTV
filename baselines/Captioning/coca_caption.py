...
def process_batch(data_batch, label_batch):
    ...
    for filepath, label in zip(data_batch, label_batch):
        ...
        # ============ sampling frames from the video with sample rate============ #
        video_frames = sample_vid(filepath, args.sample_rate, type='coca')
        # video_frames = torch.stack(video_frames).cuda() # [t, c, h, w]
        # ============ generate captions using CoCa ============ #
        # with torch.no_grad(), torch.cuda.amp.autocast():
        with torch.no_grad():
            video_caption = ' '.join([open_clip.decode(
                coca_backbone.generate(transform(frame).unsqueeze(0))[0]).split("<end_of_text>")[0].replace("<start_of_text>", "")
                                       for frame in video_frames])
            # video_feats = coca_backbone.module.encode_image(video_frames).float() # [max_seq_len, batch_size, embed_dim]
        video_caption_batch.append(video_caption)
    ...
    return video_caption_batch, hypotheses, torch.tensor(labels).cuda()
...
if __name__ == '__main__':
    ...
    path = os.path.join(os.environ['DATA_ROOT'], args.split_type)
    ckpt_file = 'coca_caption_best_{}.pth'.format(str(args.run_id))
    model_ckpt_path = os.path.join(os.getcwd(), ckpt_file)
    logger_filename = 'coca_caption_log_{}.txt'.format(str(args.run_id))
