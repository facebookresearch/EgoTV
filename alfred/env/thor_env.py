...
class ThorEnv(Controller):
    ...
    def restore_scene(self, object_poses, object_toggles, dirty_and_empty):
        ...
        if dirty_and_empty:
            ...
            for o in object_poses:
                super().step(dict(action='SetObjectStates',
                                  SetObjectStates={'objectType': o['objectName'].split('_')[0],
                                                   'stateChange': 'dirtyable', 'isDirty': True}))

                super().step(dict(action='SetObjectStates',
                                  SetObjectStates={'objectType': o['objectName'].split('_')[0],
                                                   'stateChange': 'canFillWithLiquid', 'isFilledWithLiquid': False}))
    ...
    def set_task(self, traj, args, reward_type='sparse', max_episode_length=2000):
        ...
        self.task = get_task(task_type, traj, self, args, reward_type=reward_type,
            ...
            if 'action' in action:
                ...
                if action['action'] == 'CloseObject' and "Fridge" in action['objectId']:
                    ...
                    self.cooled_objects = self.cooled_objects | set(
                        cooled_object_ids) if cooled_object_ids is not None else set()

                # if action['action'] == 'SliceObject' and "Knife" in action['objectId']:
                #     knife = get_objects_of_type('Knife', event.metadata)[0]
                #     sliced_object_ids = knife['objectIds']
                #     self.sliced_objects = self.sliced_objects | set(
                #         sliced_object_ids) if sliced_object_ids is not None else set()
    ...
    def va_interact(self, action, interact_mask=None, smooth_nav=True, mask_px_sample=1, debug=False):
        ...
        if type(interact_mask) is str and interact_mask == "NULL":
            ...
            for color_id, intersection_count in instance_counter.most_common():
                union_count = np.sum(
                    np.logical_or(np.all(instance_segs == color_id, axis=2), interact_mask.astype(bool)))
